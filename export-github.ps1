...
foreach ($doc in @('SEMANTIC_PROGRAM.md','CROSSTL_DESIGN.md','SEMANTIC_FRONTEND_V2.md','SEMANTIC_DEVELOPMENT.md','FRONTEND_UAST_CONTRACT.md','IMPLEMENTATION_MATRIX.md','UAST_MIGRATION_STATUS.md')) {
    Copy-Item -LiteralPath (Join-Path 'docs' $doc) -Destination (Join-Path $docDir $doc)
}
New-Item -ItemType Directory -Path (Join-Path $exportRoot 'packaging') -Force | Out-Null
Copy-Item -LiteralPath 'packaging\Package.appxmanifest' -Destination (Join-Path $exportRoot 'packaging\Package.appxmanifest')
...
if ($architecture) { Copy-Item -LiteralPath $architecture.FullName -Destination (Join-Path $docDir $architecture.Name) }
