...
func main() {
	...
	taint, err := backend.WriteRuntimeTaintMatrix(filepath.Clean(*out))
	if err != nil {
		panic(err)
	}
	pure, fallback, failed := 0, 0, 0
	for _, cell := range taint {
		if cell.PureNative {
			pure++
		} else if cell.RuntimeFallback {
			fallback++
		} else {
			failed++
		}
	...
	fmt.Printf("Final-source taint observations: cells=%d pure-native=%d runtime-fallback=%d error=%d (only syntax-emitting fixture contexts)\n", len(taint), pure, fallback, failed)
}
