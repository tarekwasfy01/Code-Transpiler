...
func canonicalObserved(id string) string {
	if strings.HasPrefix(id, "BINARY:") || strings.HasPrefix(id, "COMPARE:") ||
		...
		strings.HasPrefix(id, "BITWISE_BINARY:") {
		return id
	}
	switch id {
	case "ADD", "SUB", "MUL", "DIV", "POW", "REM":
		return "BINARY:" + id
...
func main() {
	...
	flag.Parse()
	if err := run(*authority, *legacy, *base, *out); err != nil {
		panic(err)
	}
}
...
func run(authorityPath, legacyPath, base, out string) error {
	...
	for _, id := range []string{"ADD", "SUB", "MUL", "DIV", "REM", "POW", "BIT_AND", "BIT_OR", "BIT_XOR", "SHL", "SHR", "EQ", "NE", "LT", "LE", "GT", "GE", "AND", "OR", "NOT", "LITERAL", "LOAD", "ASSIGNMENT", "RETURN", "ITERATION", "CALL", "APPEND"} {
		if _, ok := p0[id]; !ok {
			p0[id] = "canonical-kernel"
		}
	}
	cases, err := readCSV(filepath.Join(base, "cases.csv"))
	if err != nil {
		return err
	}
	features, err := readCSV(filepath.Join(base, "case_semantic_features.csv"))
	if err != nil {
		return err
	}
	primRows, err := readCSV(filepath.Join(base, "case_primitive_matrix.csv"))
	if err != nil {
		return err
	}
	nodes, err := readCSV(filepath.Join(base, "uast_nodes.csv"))
	if err != nil {
		return err
	}
	...
	for _, c := range cases {
		if truthy(c["parse_success"]) && truthy(c["uast_success"]) && truthy(c["structured_demand_success"]) {
			trust[c["case_id"]] = c
		}
	}
	...
	for _, f := range features {
		...
		if opsByCase[id] == nil {
			opsByCase[id] = map[string]bool{}
		}
		for _, k := range []string{"semantic_operation", "semantic_family"} {
			if v := f[k]; v != "" {
				opsByCase[id][v] = true
			}
		}
		if l := trust[id]["source_language"]; l != "" {
			if langs[l] == nil {
				langs[l] = map[string]bool{}
			}
			langs[l][id] = true
		}
	}
	...
	for _, f := range features {
		...
		if ctx[id] == nil {
			ctx[id] = map[string]map[string]bool{}
		}
	...
	type demand struct {
		caseID, lang, prim, family, param, ops, sig string
		count                                       int
	}
	...
	for _, r := range primRows {
		...
		id := canonicalObserved(r["primitive_id"])
		if !isSemanticPrimitiveID(id) {
			continue
		}
		family := r["primitive_family"]
		...
		d := demands[key]
		if d == nil {
			d = &demand{caseID: cid, lang: c["source_language"], prim: id, family: family, param: param, ops: strings.Join(opList, ";"), count: 0}
			demands[key] = d
		}
		d.count++
	}
	...
	for cid, c := range trust {
		if _, ok := ctx[cid]; !ok {
			continue
		}
		for op := range opsByCase[cid] {
			if families[op] || !isSemanticPrimitiveID(op) {
				continue
			}
			id := canonicalObserved(op)
			if id == "" {
				continue
			}
			key := cid + "|" + id + "|feature"
			if _, exists := demands[key]; !exists {
				demands[key] = &demand{caseID: cid, lang: c["source_language"], prim: id, ops: op, count: 1}
			}
		}
	}
	...
	for _, d := range demands {
		var sig []string
		sig = append(sig, d.prim, d.family, d.param, d.ops)
		for _, k := range []string{"type_model", "numeric_model", "effects", "evaluation_order", "binding", "scope", "ownership", "lifetime", "representation", "control_flow", "memory_behavior", "exception_behavior"} {
			sig = append(sig, k+"="+joinCtx(d.caseID, k))
		}
		d.sig = strings.Join(sig, "|")
		demandRows = append(demandRows, []string{d.caseID, d.lang, "", joinCtx(d.caseID, "language_operation"), joinCtx(d.caseID, "node_kind"), d.prim, d.family, d.param, d.ops, joinCtx(d.caseID, "operand_roles"), joinCtx(d.caseID, "result_role"), joinCtx(d.caseID, "effects"), joinCtx(d.caseID, "binding"), joinCtx(d.caseID, "scope"), joinCtx(d.caseID, "control_flow"), joinCtx(d.caseID, "memory_behavior"), joinCtx(d.caseID, "exception_behavior"), joinCtx(d.caseID, "evaluation_order"), joinCtx(d.caseID, "type_model"), joinCtx(d.caseID, "representation"), fmt.Sprint(d.count), d.sig})
	}
	...
	for _, d := range demands {
		id := d.prim
		mapped := id
		class := "EXACT_EXISTING"
		exact := false
		param := false
		derived := false
		if id == "" {
			class = "INVALID"
		} else if _, ok := p0[id]; ok {
			exact = true
		} else {
			// Generic family equivalence against the canonical authority.
			for pid := range p0 {
				if strings.HasPrefix(id, "BINARY:") && strings.HasPrefix(pid, "BINARY:") || strings.HasPrefix(id, "COMPARE:") && strings.HasPrefix(pid, "COMPARE:") || strings.HasPrefix(id, "LOGICAL_BINARY:") && strings.HasPrefix(pid, "LOGICAL_BINARY:") || strings.HasPrefix(id, "BITWISE_BINARY:") && strings.HasPrefix(pid, "BITWISE_BINARY:") {
					mapped = pid
					param = true
					break
				}
			...
			if !param {
				...
				case "CONTROL_TRANSFER":
					if _, ok := p0["RETURN"]; ok {
						mapped = "RETURN"
						param = true
					}
				case "STORE":
					if _, ok := p0["ASSIGNMENT"]; ok {
						mapped = "ASSIGNMENT"
						param = true
					}
				}
		...
		if mapped != "" {
			knownSemantic[mapped] = true
		}
		classCounts[class]++
		closureRows = append(closureRows, []string{d.caseID, d.prim, mapped, class, yes(exact), yes(param), yes(derived)})
		if class != "EXACT_EXISTING" && class != "PARAMETERIZED_EXISTING" && class != "DERIVED_EXISTING" {
			residualRows = append(residualRows, []string{d.caseID, d.lang, d.prim, d.family, class, d.sig})
			if classes[d.sig] == nil {
				classes[d.sig] = map[string]bool{}
			}
			classes[d.sig][d.caseID] = true
		}
	}
	if err := writeCSV(filepath.Join(out, "current_closure_matrix.csv"), []string{"witness_id", "demand_primitive", "closure_primitive", "classification", "exact_existing", "parameterized_existing", "derived_existing"}, closureRows); err != nil {
		return err
	}
	if err := writeCSV(filepath.Join(out, "semantic_residuals.csv"), []string{"witness_id", "source_language", "demand_primitive", "primitive_family", "classification", "demand_signature"}, residualRows); err != nil {
		return err
	}
	// The remaining artifacts are explicit projections of the same relation,
	// keeping a single source of truth.
	var classRows, basisRows [][]string
	i := 0
	for sig, caseset := range classes {
		i++
		var ids []string
		...
		sort.Strings(ids)
		classRows = append(classRows, []string{fmt.Sprintf("RC%04d", i), sig, fmt.Sprint(len(ids)), strings.Join(ids, ";")})
		basisRows = append(basisRows, []string{fmt.Sprintf("RC%04d", i), sig, "SEMANTIC_RESIDUAL"})
	}
	...
	for sig := range classes {
		candRows = append(candRows, []string{"CAND-" + sig, "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "GENUINELY_NEW_SEMANTIC", "false"})
		rejectRows = append(rejectRows, []string{"CAND-" + sig, "GENUINELY_NEW_SEMANTIC", "rejected_pending_productive_kernel", "no automatic atom acceptance"})
	}
	...
	for id, src := range p0 {
		...
		if knownSemantic[id] {
			status = "OBSERVED_OR_CLOSURE_REQUIRED"
		}
		redRows = append(redRows, []string{id, src, status, "false"})
	}
	for id := range knownSemantic {
		redBasis = append(redBasis, []string{id, "OBSERVED_DEMAND_BASIS", "KEEP"})
	}
...
func redRowsToSimple(in [][]string) [][]string {
	out := make([][]string, 0, len(in))
	for _, r := range in {
		if len(r) >= 4 {
			out = append(out, []string{r[0], r[2], r[3]})
		}
	}
	return out
}
