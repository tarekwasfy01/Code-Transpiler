		...
		if len(events) > 0 {
			...
			if factErr := materializeStructuredMatrixFacts(language, events, builder); factErr == nil {
				if u, uErr := BuildCanonicalUniversalASTFromFrontendFacts(builder.Facts); uErr == nil && u != nil && len(u.Nodes) > 1 {
					incomplete := false
					for i := range u.Nodes {
						common, decodeErr := decodeUniversalCommon(&u.Nodes[i])
						if decodeErr == nil && strings.HasPrefix(strings.ToLower(common.Operation.Operator), "unsupported.") {
							incomplete = true
							break
						}
					}
					...
					if u == nil {
						continueTableProjection := true
						_ = continueTableProjection
					} else {
...
func materializeStructuredMatrixFacts(language string, events []matrixir.CanonicalSemanticEvent, builder *FrontendFactsBuilder) error {
	...
	emit := func(id int, structural, kind string, event *matrixir.CanonicalSemanticEvent) error {
		...
		if event != nil {
			...
			// Literal domains are proved by the structured MatrixIR event. Keep
			// that fact in the explicit frontend type channel; it must not be
			// reconstructed later from source text.
			switch strings.ToLower(op.LiteralKind) {
			case "number":
				builder.Facts.TypesFact = append(builder.Facts.TypesFact, FrontendTypeFact{NodeID: id, Type: SemanticType{Kind: "float", Bits: 64, IEEE754: true, TypeOrigin: "inferred"}})
			case "string":
				builder.Facts.TypesFact = append(builder.Facts.TypesFact, FrontendTypeFact{NodeID: id, Type: SemanticType{Kind: "string", TypeOrigin: "inferred"}})
			case "boolean":
				builder.Facts.TypesFact = append(builder.Facts.TypesFact, FrontendTypeFact{NodeID: id, Type: SemanticType{Kind: "boolean", TypeOrigin: "inferred"}})
			}
		}
		builder.AddNode(n)
