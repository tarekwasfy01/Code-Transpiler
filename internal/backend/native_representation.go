...
func SolveNativeLayout(profile NativeTargetProfile, typ SemanticType, context NativeABIContext) (NativeLayout, error) {
	...
	case "pointer", "reference", "optional", "channel":
		// Channels are opaque runtime handles at the native boundary. Their
		// synchronization semantics remain in the runtime contract; the ABI
		// representation is one target-sized address.
