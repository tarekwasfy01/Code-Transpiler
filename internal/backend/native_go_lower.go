...
// LowerNativeGo lowers a bounded scalar Go subset directly from Go's AST.
...
func LowerNativeGo(filename, source string) (*SemanticProgram, error) {
	...
	program.Metadata = map[string]string{"frontend": "native-go-uast-v1", "subset": "bool,string,fixed-width-integer,control,acyclic-functions,fmt.Println(scalar)", "typecheck_architecture": "gc/amd64", "source_mapping": "go-hir", "source": "go-hir", "semantic_facts": strconv.Itoa(facts.Evidence)}
...
func (l *goScalarLowerer) function(fn *ast.FuncDecl) SemanticStatement {
	...
	for i := 0; i < signature.Results().Len(); i++ {
		...
		if r.Name() != "" || !l.scalar(r.Type()) {
			l.fail(fn, "named or non-scalar function result")
		}
	}
	...
	if fn.Type.Params != nil {
		for _, field := range fn.Type.Params.List {
			...
			for _, name := range field.Names {
				...
				if object == nil || !l.scalar(object.Type()) {
					l.fail(name, "non-scalar parameter")
				}
				symbol := l.name(name)
				typ := l.types[symbol]
...
func (l *goScalarLowerer) expr(n ast.Expr) *SemanticExpression {
	...
	if !l.scalar(l.info.TypeOf(n)) {
		l.fail(n, "only bool, string and fixed-width integers have a verified lowering")
		return e
	}
	...
	case *ast.UnaryExpr:
		...
		e.Value = l.expr(x.X)
	case *ast.BinaryExpr:
		if x.Op != gotoken.EQL && x.Op != gotoken.NEQ && x.Op != gotoken.LAND && x.Op != gotoken.LOR {
	...
	case *ast.ExprStmt:
		...
		if ok {
			if _, ok := call.Fun.(*ast.Ident); ok {
				s.Kind = "expression"
				s.Expression = l.helperCall(call)
				break
			}
		}
		if !ok || len(call.Args) != 1 || call.Ellipsis.IsValid() {
			l.fail(n, "only fmt.Println(string) expression statements are supported")
			break
		}
		sel, ok := call.Fun.(*ast.SelectorExpr)
		if !ok {
			l.fail(n, "unsupported call")
			break
		}
		object := l.info.Uses[sel.Sel]
		fn, ok := object.(*types.Func)
		if !ok || fn.Pkg() == nil || fn.Pkg().Path() != "fmt" || fn.Name() != "Println" {
			l.fail(n, "only fmt.Println is supported")
			break
		}
		typ, ok := l.info.TypeOf(call.Args[0]).(*types.Basic)
		integer, isInteger := nativeFixedInteger(l.info.TypeOf(call.Args[0]))
		if !ok || (typ.Info()&types.IsString == 0 && !isInteger) {
			l.fail(n, "Println argument must be string or fixed-width integer")
			break
		}
		argument := l.expr(call.Args[0])
		if isInteger {
			argument = l.integerOperation(call.Args[0], "integer.format", integer, argument)
		}
		s.Kind = "expression"
		s.Expression = &SemanticExpression{Kind: "call", Operator: "eager_left_to_right", Source: l.span(call), Value: &SemanticExpression{Kind: "identifier", Name: "print", Source: l.span(call.Fun)}, Arguments: []SemanticArgument{{Value: argument}}}
	default:
