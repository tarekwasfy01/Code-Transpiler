...
func RegisteredTargetSpecs() []TargetSpec {
	...
	for _, backend := range Backends() {
		...
		specs = append(specs, TargetSpec{
			...
			Hooks: []string{"NativeEmitter", "mainOpen", "mainClose"}, Operators: universalOperatorSpecs(),
			Types:    map[string]string{"number": "number", "boolean": "boolean", "string": "string", "null": "null"},
			Literals: targetLiteralSpecExisting(backend.ID),
			Naming:   TargetNamingSpec{CaseSensitive: true, StyleInsensitive: styleInsensitive, GeneratedPrefix: prefix}, Imports: TargetImportSpec{Ordering: "lexical"},
...
func targetProjectionFormsExisting(target string) map[string]TargetProjectionSpec {
	// TargetSpecs describe native syntactic availability. The canonical UAST
	// projector either emits through these forms or returns
	// DIRECT_NATIVE_UNAVAILABLE; it does not select a runtime form.
	coreMode := PreservationDirect
	return map[string]TargetProjectionSpec{
		projectionFormCore:      {Mode: coreMode},
		projectionFormAggregate: {Mode: coreMode},
		projectionFormVariable:  {Mode: PreservationDirect},
		projectionFormDeclGroup: {Mode: coreMode},
		...
		// Kept for the explicit legacy compatibility entry only. The canonical
		// UAST projector rejects this form before target source is emitted.
...
func (p UniversalTargetProjector) Project(u *UniversalASTDocument, spec TargetSpec) (Doc, error) {
	...
	if err != nil {
		...
		return DocText{Text: source}, nil
	}
