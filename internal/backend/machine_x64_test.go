...
func TestMachineDataNamesKeepReadonlyBeforeMutable(t *testing.T) {
	got := orderedMachineDataNames([]string{"uast_global_z", "uast_const_b", "__project_data_a", "uast_const_a"})
	want := []string{"uast_const_a", "uast_const_b", "__project_data_a", "uast_global_z"}
	if len(got) != len(want) {
		t.Fatalf("ordered data names length = %v, want %v", got, want)
	}
	for i := range want {
		if got[i] != want[i] {
			t.Fatalf("ordered data names = %v, want %v", got, want)
		}
	}
}
