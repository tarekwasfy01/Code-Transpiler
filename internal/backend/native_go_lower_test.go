...
func TestNativeGoNilLowersAsNullLiteral(t *testing.T) {
	program, err := LowerNativeGo("nil-literal.go", `package main
func unavailable() (any, error) { return nil, nil }
`)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !bytes.Contains(encoded, []byte(`"literal_kind":"null"`)) {
		t.Fatal("Go nil was not preserved as a null literal")
	}
	if bytes.Contains(encoded, []byte(`"name":"native_var_nil"`)) {
		t.Fatal("Go nil was incorrectly emitted as a symbol reference")
	}
}

func TestNativeGoNilToSliceConversionPreservesSliceType(t *testing.T) {
	program, err := LowerNativeGo("nil-slice-conversion.go", `package main
...
`)
	...
	if bytes.Contains(encoded, []byte(`"name":"native_var_string"`)) {
		t.Fatal("slice type expression was lowered as a callable identifier")
	}
	if !bytes.Contains(encoded, []byte(`"kind":"slice"`)) || !bytes.Contains(encoded, []byte(`"literal_kind":"null"`)) {
		t.Fatal("typed nil slice conversion lost its null value or slice type")
	}
}
...
func TestNativeGoStructLiteralKeysAreFieldNamesNotBindings(t *testing.T) {
	program, err := LowerNativeGo("keyed-struct.go", `package main
type record struct { ID string; Count int }
func main() { _ = []record{{ID: "x", Count: 1}} }
`)
	if err != nil {
		t.Fatal(err)
	}
	...
	if bytes.Contains(encoded, []byte(`"name":"native_var_ID"`)) || bytes.Contains(encoded, []byte(`"name":"native_var_Count"`)) {
		t.Fatalf("struct field labels were lowered as unresolved variable bindings: %s", encoded)
	}
	if !bytes.Contains(encoded, []byte(`"text":"\"ID\""`)) || !bytes.Contains(encoded, []byte(`"text":"\"Count\""`)) {
		t.Fatal("struct field labels were not retained as string-valued field identities")
	}
}

func TestNativeGoCSharpContractCatalogStructKeysAreFieldNames(t *testing.T) {
	const filename = "csharp_go2cs_contracts.go"
	source, err := os.ReadFile(filename)
	if err != nil {
		t.Skipf("repository fixture unavailable: %v", err)
	}
	program, err := LowerNativeGo(filename, string(source))
	if err != nil {
		t.Fatal(err)
	}
	encoded, err := program.MarshalSemanticJSON()
	...
	for _, synthetic := range []string{
		`"name":"native_var_ID"`, `"name":"native_var_Family"`,
		`"name":"native_var_RequiredUASTFacts"`, `"name":"native_var_RequiredBackendFacts"`,
		`"name":"native_var_Status"`, `"name":"f_Extensions"`, `"name":"item_ID"`,
		`"name":"native_var_csharpGo2CSEvidenceCatalog"`,
	} {
		if bytes.Contains(encoded, []byte(synthetic)) {
			t.Errorf("partial type information produced synthetic unresolved identifier %s", synthetic)
		}
	}
}
...
func TestLowerNativeGoPackageSharesTypecheckAndKeepsPerFileBodies(t *testing.T) {
	...
	helperPath := filepath.Join(filepath.Dir(mainPath), "helper.go")
	programs := make(map[string]*SemanticProgram, 2)
	err := LowerNativeGoSourcePackageWithWorkers([]NativeGoSourceFile{
		{Filename: mainPath, Source: "package sample\nfunc main() { _ = helper(41) }\n"},
		{Filename: helperPath, Source: "package sample\nfunc helper(x int64) int64 { return x + 1 }\n"},
	}, 2, func(unit NativeGoSourceFile, program *SemanticProgram, lowerErr error) {
		if lowerErr != nil {
			t.Errorf("lower %s: %v", unit.Filename, lowerErr)
			return
		}
		programs[unit.Filename] = program
	})
	if err != nil {
		t.Fatal(err)
	}
