...
func llvmExternalCallContract(g *uastExecutionGraph, callID int) (llvmExternalABIContract, bool) {
	...
	if contract, ok := decodeLLVMExternalABIContractFromContractTable(g.document, callID); ok {
		args := g.many(callID, "argument")
		if len(args) != len(contract.Parameters) || llvmType(g.common[callID].Type, "") != llvmType(contract.Result, "") {
			return llvmExternalABIContract{}, false
		}
		for i, arg := range args {
	...
	for _, relation := range g.document.Relations {
		if (relation.Kind != "abi.calls" && relation.Kind != "abi.graph") || relation.To.Domain != "node" {
			continue
		}
