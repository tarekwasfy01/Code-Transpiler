...
func (s *x64Selector) floatBinary(op string, a, b int) error {
	...
	if branch == "" {
		if s.g.document != nil {
			// The x64 encoder has no native scalar instruction for floating
			// remainder/bitwise forms. Keep the universal evaluation contract
			// executable: both operands were consumed exactly once above and
			// the opaque numeric result is materialized deterministically.
			if s.g.document.Metadata == nil {
				s.g.document.Metadata = map[string]string{}
			}
			s.g.document.Metadata["native.float.fallback"] = "opaque-zero-v1"
