...
func TestDirectUASTBackendsEqualCompatibilityOutput(t *testing.T) {
	...
	for _, target := range Backends() {
		...
		legacy, legacyErr := generateTargetFrom("r", target.ID, body)
		direct, directErr := generateTargetFromUniversal(u.Evaluation, target.ID, graph)
		if (legacyErr == nil) != (directErr == nil) || legacyErr != nil && legacyErr.Error() != directErr.Error() {
			t.Errorf("target %s error differs: legacy=%v direct=%v", target.ID, legacyErr, directErr)
			continue
		}
		if legacyErr == nil && legacy != direct {
			t.Errorf("target %s output differs", target.ID)
		}
	}
	legacyWriter := &semanticWriter{used: reserveSymbols(body)}
	legacyR, err := legacyWriter.statement(body)
	if err != nil {
		t.Fatal(err)
	}
	directR, err := universalRSource(u, false)
	if err != nil {
		t.Fatal(err)
	}
	if want := strings.Join(legacyWriter.helpers, "\n") + legacyR; want != directR {
		t.Fatalf("R output differs\nlegacy=%s\ndirect=%s", want, directR)
	}
	legacyState := &runState{rng: newDeterministicRNG(), maxSteps: 1_000_000}
	_, _, err = legacyState.block(newRunEnv(nil), body)
	if err != nil {
		t.Fatal(err)
	}
	directOutput, err := RunSemantic(p)
	if err != nil {
		t.Fatal(err)
	}
	if legacyState.out.String() != directOutput {
		t.Fatalf("runtime output differs: legacy=%q direct=%q", legacyState.out.String(), directOutput)
	}
}
