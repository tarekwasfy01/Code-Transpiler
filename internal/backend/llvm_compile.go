...
func CompileLLVMProject(p *SemanticProject, opts LLVMCompileOptions) (LLVMProjectResult, error) {
	...
	if opts.OutputKind == LLVMExecutable && !opts.EmbedAllModules {
...
func (e *llvmEmitter) discoverFunctions() error {
	...
	anonymousOrdinal := 0
	for _, id := range ids {
		if contract, ok := llvmExternalFunctionContract(e.g, id); ok {
			if existing, exists := e.externalCalls[contract.Symbol]; exists && !llvmExternalABIEqual(existing, contract) {
				return fmt.Errorf("external function node %d conflicts for symbol %q", id, contract.Symbol)
			}
...
// llvmExternalFunctionContract recognizes a bodyless declaration only when a
// canonical ABI contract explicitly links it to a hosted import.
func llvmExternalFunctionContract(g *uastExecutionGraph, functionID int) (llvmExternalABIContract, bool) {
	if g == nil || g.document == nil {
		return llvmExternalABIContract{}, false
	}
	if contract, ok := decodeLLVMExternalABIContractFromContractTable(g.document, functionID); ok {
		return contract, true
	}
	for _, relation := range g.document.Relations {
		if relation.Kind != "abi.graph" || relation.From != functionID || relation.To.Domain != "node" {
			continue
		}
		contractID, err := strconv.Atoi(relation.To.ID)
		if err != nil || !llvmABIContractNode(g, contractID) {
			continue
		}
		if contract, ok := decodeLLVMExternalABIContract(g.nodes[contractID]); ok {
			return contract, true
		}
	}
	return llvmExternalABIContract{}, false
...
func llvmGeneratedRuntimeSymbol(symbol string) bool {
	switch symbol {
	case "log_SetFlags", "log_SetPrefix", "buildcfg_Check", "fmt_Fprintf", "os_Exit", "gc_Main", "base_Exit", "__type_assert", "_fltused":
		return true
	default:
		return false
	}
...
func llvmGenericBuiltinCallContract(g *uastExecutionGraph, callID int, symbol string) (llvmExternalABIContract, bool) {
	if symbol == "__type_assert" {
		args := g.many(callID, "argument")
		params := make([]SemanticType, len(args))
		for i := range params {
			params[i] = SemanticType{Kind: "integer", Bits: 64, Signed: boolPtr(true), TypeOrigin: "derived"}
		}
		return llvmExternalABIContract{Symbol: "__type_assert", CallingConvention: "ccc", Parameters: params, Result: SemanticType{Kind: "integer", Bits: 64, Signed: boolPtr(true), TypeOrigin: "derived"}}, true
	}
	args := g.many(callID, "argument")
	params := make([]SemanticType, len(args))
...
func (e *llvmEmitter) emitBuiltinRuntime() error {
	...
	e.emitLine("define i64 @__type_assert(...) { ret i64 1 }")
...
func (e *llvmEmitter) emitExpr(id int) (llvmValue, error) {
	...
	case "unary", "iteration":
		...
		case "*", "deref":
			...
			// The dereference node's own type is normally the result type, not
			// the pointer layout. Prefer the operand's declared pointer element
			// when it is available; using derefType.Element here lost valid
			// pointee facts for `*p` where the result is a scalar or record.
			if operandType, operandErr := e.semanticTypeForNode(child); operandErr == nil && operandType.Element != nil && (strings.EqualFold(operandType.Kind, "pointer") || strings.EqualFold(operandType.Kind, "reference")) {
				pointeeType = *operandType.Element
			} else if derefType.Element != nil && (strings.EqualFold(derefType.Kind, "pointer") || strings.EqualFold(derefType.Kind, "reference")) {
	...
	default:
		...
		case "AddressOf", "Deref":
			...
			pointeeType := derefType
			if operandType, operandErr := e.semanticTypeForNode(value); operandErr == nil && operandType.Element != nil && (strings.EqualFold(operandType.Kind, "pointer") || strings.EqualFold(operandType.Kind, "reference")) {
				pointeeType = *operandType.Element
			} else if derefType.Element != nil && (strings.EqualFold(derefType.Kind, "pointer") || strings.EqualFold(derefType.Kind, "reference")) {
				pointeeType = *derefType.Element
			}
