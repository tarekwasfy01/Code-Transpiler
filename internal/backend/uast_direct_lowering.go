...
func directLoweringRequirements(rows []map[string]string) (DirectLoweringAnalysis, error) {
	...
	for _, source := range rows {
		...
		// The current TargetSpec already proves this candidate as direct. It is
		// not a residual and therefore must not manufacture a fake runtime
		// obligation merely to keep an old promotion report populated.
		if len(missing) == 0 {
			continue
		}
