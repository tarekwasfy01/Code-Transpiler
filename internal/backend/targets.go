...
static class R2{public sealed class Value{public readonly double N;public Value(double n){N=n;}public override string ToString(){return (!double.IsNaN(N)&&!double.IsInfinity(N)&&N==Math.Truncate(N)&&Math.Abs(N)<9e15)?((long)N).ToString():N.ToString(System.Globalization.CultureInfo.InvariantCulture);}}public static readonly object Null=null;public static double Num(object v){Value value=v as Value;if(value!=null)return value.N;IConvertible convertible=v as IConvertible;if(convertible!=null)return convertible.ToDouble(System.Globalization.CultureInfo.InvariantCulture);return double.NaN;}public static bool Truth(object v){if(v==null)return false;if(v is bool)return (bool)v;return true;}public static IEnumerable<object> Iter(object v){object[] values=v as object[];return values??new object[]{v};}public static void Discard(object value){}public static object Call(string kernel,string n,object[]a){if(n.StartsWith("__binary_")){var op=n.Substring(9);var x=Num(a[0]);var y=Num(a[1]);switch(op){case "+":return new Value(x+y);case "-":return new Value(x-y);case "*":return new Value(x*y);case "/":return new Value(x/y);case "==":return a[0] is string&&a[1] is string?Equals(a[0],a[1]):x==y;case "!=":return a[0] is string&&a[1] is string?!Equals(a[0],a[1]):x!=y;case "<":return x<y;case "<=":return x<=y;case ">":return x>y;case ">=":return x>=y;}}if(n=="c"||n=="list")return a;if(n=="print"){var v=a.Length>0?a[0]:null;Console.Write(v);Console.Write("\n");return v;}if(kernel=="predicate"||kernel=="numeric-predicate"||kernel=="missingness")return false;if(kernel=="random")return new Value(.5);return a.Length>0?a[0]:null;}}
`
