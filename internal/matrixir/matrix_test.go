...
func TestNimParameterlessRoutineHeaders(t *testing.T) {
	for _, source := range []string{"proc foo =\n  discard\n", "proc bar\n"} {
		if _, err := Canonicalize("nim", source); err != nil {
			t.Fatalf("Nim routine header rejected %q: %v", source, err)
		}
	}
}
