...
func TraceCanonicalize(language, source string) (CanonicalProgram, []LegacyTraceRow, error) {
	...
	if err != nil {
		return CanonicalProgram{}, nil, err
	}
	...
	for _, e := range p.SemanticEvents {
		...
		if len(e.Roles) == 0 {
			rows = append(rows, LegacyTraceRow{Language: language, ProducedNode: e.ID, SemanticEvent: e.ID, ParserContext: e.StructureKind})
		}
	}
	return p, rows, nil
}
