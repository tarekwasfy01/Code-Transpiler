...
const allTokens=[],allTokenClasses=[],fixtureRanges=[],controlEdges=[];
...
for(let f=0,offset=0;f<analysis.length;f++){
 const a=analysis[f],nodes=rowsOf(a.lexical_node_matrix).slice(1),control=rowsOf(a.lexical_control_matrix).slice(1).map(r=>r.slice(1));
 if(nodes.length!==a.token_classes.length)throw new Error(`token dimension ${f}`);
 fixtureRanges.push({offset,count:nodes.length});allTokens.push(...nodes);allTokenClasses.push(...a.token_classes);
...
const tokenCount=allTokens.length,fixtureSemanticCount=zeros(fixtures.length,semantics.length),fixtureSemanticBinary=zeros(fixtures.length,semantics.length),fixtureClassCount=zeros(fixtures.length,tokenClasses.length);
fixtureRanges.forEach((range,f)=>{for(let i=range.offset;i<range.offset+range.count;i++){for(let s=0;s<semantics.length;s++){fixtureSemanticCount[f][s]+=allTokens[i][s];if(allTokens[i][s])fixtureSemanticBinary[f][s]=1;}fixtureClassCount[f][allTokenClasses[i]]++;}});
...
const design=routes.map((_,i)=>[...routeSemanticBinary[i],...routeSemanticCount[i],...routeRelations[i],...stageFailure[i],...stageKnown[i],...sourceIncidence[i],...targetIncidence[i]]);
const designKnown=routes.map((_,i)=>[...Array(semantics.length*2+relations.length).fill(1),...stageKnown[i],...Array(stages.length+languages.length*2).fill(1)]);
...
const result={schema_version:3,input:{measurement_source_hash:measurements.summary.source_tree_hash,measurements_sha256:createHash('sha256').update(await fs.readFile(path.join(dir,'measurements.json'))).digest('hex')},
 dimensions:{fixtures:fixtures.length,routes:routes.length,tokens:tokenCount,semantic_dimensions:semantics.length,token_classes:tokenClasses.length,relations:relations.length,stages:stages.length,design_columns:design[0].length,design_cells:design.length*design[0].length,gram_cells:gram.length*gram[0].length,sparse_control_edges:controlEdges.length},
 axes:{languages,features,semantics,token_classes:tokenClasses,relations,stages,design_columns:[...semantics.map(s=>'has_'+s),...semantics.map(s=>'count_'+s),...relations.map(r=>'edges_'+r),...stages.map(s=>'fail_'+s),...stages.map(s=>'known_'+s),...languages.map(l=>'source_'+l),...languages.map(l=>'target_'+l)]},
 matrices:{token_semantic:allTokens,token_class:allTokenClasses,fixture_token_incidence_sparse:fixtureRanges,token_control_sparse:controlEdges,fixture_semantic_count:fixtureSemanticCount,fixture_semantic_binary:fixtureSemanticBinary,fixture_token_class_count:fixtureClassCount,route_fixture_projection_sparse:routeFixtureIndex,route_semantic_binary:routeSemanticBinary,route_semantic_count:routeSemanticCount,route_lexical_relation_count:routeRelations,stage_failure:stageFailure,stage_known:stageKnown,source_incidence:sourceIncidence,target_incidence:targetIncidence,design,design_known:designKnown,gram,known_gram:knownGram,semantic_transition:semanticTransitions,source_semantic_transition:sourceSemanticTransitions,token_class_transition:classTransitions,semantic_stage_failure:semanticStageFailure,semantic_stage_known:semanticStageKnown},
...
await fs.writeFile(path.join(dir,'semantic_matrix_v3.json'),JSON.stringify(result,null,2));
await fs.writeFile(path.join(dir,'semantic_matrix_v3_summary.json'),JSON.stringify({schema_version:3,input:result.input,dimensions:result.dimensions,summary:result.summary,rules:result.rules},null,2));
