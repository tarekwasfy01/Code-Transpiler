...
$env:GOCACHE = Join-Path $repoRoot '.audit-cache\go-build'
$env:CGO_ENABLED = '1'
...
$iconResource = Join-Path $repoRoot 'cmd\r2many\r2many_windows.syso'
...
try {
    ...
    if (-not [string]::IsNullOrWhiteSpace($gitCommit)) { $commit = $gitCommit }
} catch {
