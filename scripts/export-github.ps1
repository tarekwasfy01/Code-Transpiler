...
$exportRoot = Join-Path $projectRoot 'github-release\Code-Transpiler'
$resolvedWorkspace = (Resolve-Path -LiteralPath $projectRoot).Path
...
$files = @(
    '.gitignore', 'LICENSE', 'README.md', 'docs\\SEMANTIC_PROGRAM.md',
    'docs\\CROSSTL_DESIGN.md', 'docs\\SEMANTIC_FRONTEND_V2.md', 'docs\\SEMANTIC_DEVELOPMENT.md',
    'docs\\FRONTEND_UAST_CONTRACT.md', 'docs\\IMPLEMENTATION_MATRIX.md', 'docs\\UAST_MIGRATION_STATUS.md',
    'docs\\THIRD_PARTY_NOTICES.md', 'THIRD_PARTY_NOTICES.txt',
    'Package.appxmanifest',
    'go.mod', 'go.sum', 'code_transpiler.go', 'code_transpiler_test.go',
    'scripts\\build-onefile.ps1', 'build-code-transpiler.bat', 'build-universal-code-transpiler.bat',
    'scripts\\export-github.ps1'
)
foreach ($file in $files) { Copy-Item -LiteralPath $file -Destination (Join-Path $exportRoot $file) }
$architecture = Get-ChildItem -LiteralPath $projectRoot -File | Where-Object { $_.Name -like '*Canonical Architecture.md' } | Select-Object -First 1
if ($architecture) { Copy-Item -LiteralPath $architecture.FullName -Destination (Join-Path $exportRoot 'UNIVERSAL_CANONICAL_ARCHITECTURE.md') }
...
foreach ($dir in @('.github', 'cmd', 'internal', 'assets', 'cpp_runtime', 'licenses', 'scripts', 'tools', 'docs', 'tests', 'oracle', 'examples')) {
